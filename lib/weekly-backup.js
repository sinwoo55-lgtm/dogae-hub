...
import { backupItemId, backupStateDifferences, isRestorableBackupId, isRestoreLockActive, koreaDateKey, recentRestoreResults, restorePlan } from './backup-utils.js';

const BACKUPS = db.collection('weekly_backups');
const RESTORE_LOCK = BACKUPS.doc('_control').collection('locks').doc('active');
const RETAIN_COUNT = 8;
const EXCLUDED_ROOT_COLLECTIONS = new Set(['weekly_backups']);
const RESTORE_LOCK_MS = 15 * 60 * 1000;
...
async function acquireRestoreLock(backupId) {
  const holder = `restore-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  const now = Date.now();
  await db.runTransaction(async (transaction) => {
    const active = await transaction.get(RESTORE_LOCK);
    if (active.exists && isRestoreLockActive(active.data().expiresAt, now)) {
      const error = new Error('다른 백업 복원이 진행 중입니다. 완료 후 다시 시도하세요.');
      error.code = 'restore-in-progress';
      throw error;
    }
    transaction.set(RESTORE_LOCK, { holder, backupId, startedAt: FieldValue.serverTimestamp(), expiresAt: now + RESTORE_LOCK_MS });
  });
  return holder;
}

async function releaseRestoreLock(holder) {
  await db.runTransaction(async (transaction) => {
    const active = await transaction.get(RESTORE_LOCK);
    if (active.exists && active.data().holder === holder) transaction.delete(RESTORE_LOCK);
  });
}

export async function assertNoRestoreInProgress() {
  const active = await RESTORE_LOCK.get();
  if (active.exists && isRestoreLockActive(active.data().expiresAt)) {
    const error = new Error('백업 복원이 진행 중이라 지금은 데이터를 수정할 수 없습니다. 잠시 후 다시 시도하세요.');
    error.code = 'restore-in-progress';
    throw error;
  }
}
...
export async function restoreWeeklyBackup(backupId) {
  const lockHolder = await acquireRestoreLock(backupId);
  let result = null;
  let safetyBackupId = null;
  try {
    const preview = await previewWeeklyBackup(backupId);
    const backup = BACKUPS.doc(backupId);
    result = backup.collection('restore_results').doc(`restore-${Date.now()}`);
    ...
    if (result) await result.set({ status: 'failed', safetyBackupId, verification: error?.verification || null, completedAt: FieldValue.serverTimestamp(), error: error instanceof Error ? error.message.slice(0, 500) : '알 수 없는 오류' }, { merge: true });
    if (error && typeof error === 'object') error.safetyBackupId = safetyBackupId;
    throw error;
  } finally {
    try { await releaseRestoreLock(lockHolder); }
    catch (error) { console.error('weekly backup restore lock release error', error); }
  }
}
