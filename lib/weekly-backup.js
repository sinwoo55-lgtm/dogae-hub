...
import { backupItemId, isRestorableBackupId, koreaDateKey, recentRestoreResults, restorePlan } from './backup-utils.js';
...
export async function previewWeeklyBackup(backupId) {
  ...
  const backupPaths = items.docs.map((item) => item.data().sourcePath);
  if (backupPaths.some((path) => !isSourcePath(path)) || new Set(backupPaths).size !== backupPaths.length) throw new Error('백업 데이터 경로가 올바르지 않습니다.');
  const currentRecords = [];
  const collections = await db.listCollections();
  for (const collection of collections) {
    if (!EXCLUDED_ROOT_COLLECTIONS.has(collection.id)) await collectDocuments(collection, currentRecords);
  }
...
export async function restoreWeeklyBackup(backupId) {
  ...
  try {
    ...
    const valuesByPath = new Map(items.docs.map((item) => [item.data().sourcePath, item.data().data]));
    ...
    await result.set({ status: 'completed', completedAt: FieldValue.serverTimestamp(), error: null }, { merge: true });
    await backup.set({ lastRestoredAt: FieldValue.serverTimestamp(), lastSafetyBackupId: safetyBackupId }, { merge: true });
    return { backupId, safetyBackupId, restoreCount: preview.restoreCount, deleteCount: preview.deleteCount, documentCount: preview.documentCount, status: 'completed' };
  } catch (error) {
    console.error('weekly backup restore error', error);
    await result.set({ status: 'failed', safetyBackupId, completedAt: FieldValue.serverTimestamp(), error: error instanceof Error ? error.message.slice(0, 500) : '알 수 없는 오류' }, { merge: true });
