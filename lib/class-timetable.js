...
function fontFamily(value) {
  const font = shortText(value ?? 'Noto Sans KR', 100);
  return font && !/[;{}<>\r\n]/.test(font) ? font : null;
}
...
export function classTimetableData(value) {
  ...
  const font = fontFamily(value.fontFamily);
  if (!title || !theme || image === null || !Number.isFinite(opacity) || opacity < 0 || opacity > 1 || !Number.isFinite(cellOpacity) || cellOpacity < 0 || cellOpacity > 1 || Object.values(normalizedColors).some((color) => !color) || !imageFit || !imagePosition || !font || !Array.isArray(value.cells) || value.cells.length !== CELL_COUNT) return null;
  ...
  return { title, theme, backgroundImage: image, backgroundOpacity: opacity, cellOpacity, colors: normalizedColors, imageFit, imagePosition, fontFamily: font, cells };
}
