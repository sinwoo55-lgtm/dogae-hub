...
import { isSchoolGuardEligible } from './school-guard-eligibility.js';
...
export async function syncSchoolGuardRoster() {
  ...
  const excludedMiddleSchoolCount = candidates.filter((student) => !isSchoolGuardEligible(student)).length;
  const students = deduplicateRoster(candidates.filter(isSchoolGuardEligible));
  ...
  await META.set({ syncedAt: FieldValue.serverTimestamp(), studentCount: students.length, excludedMiddleSchoolCount, lastResult: 'success', lastError: null }, { merge: true });
  return { synced: true, students: students.length, excludedMiddleSchoolCount };
}
